...
/*  Boundary nodes x = 1, x = Nx-1 */
    ...
    for(j = 0; j < parms.ny; j++){
        for(k = 0; k < parms.Nz; k++){
            ...
            data[n*parms.Nvar+2] /= Vavg;
            data[n*parms.Nvar+3] /= Vavg;
        }
    }
    free(tphi); free(tp);
    return parms;
...
{
    int i, j, k, n;
    double kr = 1.0, Da, c0;        // Rate constant = 1
    double DSh = SH/parms.Pe;
    for(i = 0; i < parms.nx; i++){
        for(j = 0; j < parms.ny; j++){
            for(k = 0; k < parms.Nz; k++){
                n = ((i*parms.ny+j)*parms.Nz)+k;
                Da = parms.Da*kr*data[n*parms.Nvar]/DSh;
                c0 = data[n*parms.Nvar+4]/(1.0 + Da);
                r[n] = DSh*S(data[n*parms.Nvar])*(data[n*parms.Nvar+4] - c0)/data[n*parms.Nvar];
                dr[n] = DSh*S(data[n*parms.Nvar])*(1.0 - 1.0/(1.0 + Da))/data[n*parms.Nvar];
#ifdef TLIM
