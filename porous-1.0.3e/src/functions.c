...
int *spiral(struct parms parms)
{
    ...
    int *sp = malloc(sizeof(int)*((m+2)*(n+2)));
    s[0] = (int*)(s + m);
    ...
    for(i = 1; i < m+1; i++){
        for(j = 1; j < n+1; j++){
            sp[i*(n+2)+j] = m*n-s[i-1][j-1];
        }
    }
    for (j = 0; j < n+2; j++){
        sp[j] = (m*n)+j;
        sp[(m+1)*(n+2)+j] = (m+2)*(n+1)+1-j;
    }
    for (i = 1; i < m+2; i++){
        sp[i*(n+2)] = (m+2)*(n+2)-i;
        sp[(i+1)*(n+2)-1] = (m+1)*(n)+1+i;
    }
    free(s);
    return (sp);
}
...
double *padding(double *data, int *sp, int dtype, struct parms parms)
{
    ...
    for (i = 0; i < parms.nx; i++){
        for (j = 0; j < parms.ny; j++){
            spij = sp[(i+1)*(parms.ny+2)+j+1]*parms.Nz;
            for (k = 0 ; k < parms.Nz; k++){
                temp[spij+k] = data[((i*parms.ny+j)*parms.Nz+k)*parms.Nvar+dtype];
            }
        }
    }
    ...
    inn = sp[2*(n+2)+1];
    double *add_send[5] = {&temp[inn], &temp[inn+m-2], &temp[inn+m+n-3],&temp[inn+2*m+n-4],&temp[inn+2*(m+n)-5]};
    double *add_recv[5] = {&temp[n*(m+1)+3], &temp[m*n+1], &temp[(m+1)*(n+1)+n+3], &temp[(m+1)*(n+1)+2], &temp[n*(m+1)+2]};

    // Down
    MPI_Sendrecv(add_send[0], parms.nx-1, MPI_DOUBLE, parms.D, 1, add_recv[0], parms.nx-1, MPI_DOUBLE, parms.U, 1, MPI_COMM_WORLD, &status);

    // Right
    MPI_Sendrecv(add_send[1], parms.ny, MPI_DOUBLE, parms.R, 2, add_recv[1], parms.ny, MPI_DOUBLE, parms.L, 2, MPI_COMM_WORLD, &status);

    // Up
    MPI_Sendrecv(add_send[2], parms.nx, MPI_DOUBLE, parms.U, 3, add_recv[2], parms.nx, MPI_DOUBLE, parms.D, 3, MPI_COMM_WORLD, &status);

    // Left
    MPI_Sendrecv(add_send[3], parms.ny, MPI_DOUBLE, parms.L, 4, add_recv[3], parms.ny, MPI_DOUBLE, parms.R, 4, MPI_COMM_WORLD, &status);

    // Odd
    MPI_Sendrecv(add_send[4], 1, MPI_DOUBLE, parms.D, 5, add_recv[4], 1, MPI_DOUBLE, parms.U, 5, MPI_COMM_WORLD, &status);

    return(temp);
}


// Create coefficient matrix for solving pressure
    ...
    for(i = 0; i < parms.nx; i++){
        for(j = 0; j < parms.ny; j++){
            for(k = 0; k < parms.Nz; k++){
                ...
                Ab[parms.N*parms.nsten+no] = dr[n]*data[n*parms.Nvar+4] - r[n];
                Vx = data[n*parms.Nvar+2]/parms.dx;
                Vy = data[n*parms.Nvar+3]/parms.dx;
                if (parms.x0+i == 0){
                    //Ab[nn*parms.nsten+l] = 0;
                    Ab[no*parms.nsten] = 1;
...
void erode(double *data, double *r, double *dr, int *sp, struct parms parms)
{
    ...
    writedata(xc, data, 4, 1, parms);           // Write concentration to data @ 1 in reverse order
    fluxC(data, r, dr, parms);                  // Update reaction rates
